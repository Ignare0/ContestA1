...
- M1：`basic01–03`，覆盖组合逻辑、时钟、blocking 和 NBA。
- M2：`basic04–05`、`alu`、`priority_encoder`，覆盖参数、generate、循环和 case。
- M3：`ip`、`axis_fifo`，覆盖层次、memory 和握手。
- M4：`i2c`，覆盖四态、多驱动、双边沿和高阻态。
- M5：`sha256`、`GEMM` 单线程正确。
- M6：完整指纹缓存、模块级增量和性能优化。
